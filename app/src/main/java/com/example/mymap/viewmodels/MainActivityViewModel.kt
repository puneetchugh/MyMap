...
class MainActivityViewModel @Inject constructor(private val apiService: ApiService,
    ...
    fun getAllPinsList() : List<model.Pin>{
        ...
        pinsList.forEach{pin -> this@MainActivityViewModel.pinDao.insert(pin)
    ...
    private fun storeInDb(pinList : List<model.Pin>){
        pinList.forEach{pin -> this@MainActivityViewModel.pinDao.insert(pin)
            Log.d(LOG_TAG, "location : "+pin.name)
        }
